...
def bose_mapping(n, x, i, als_first_row):
    if i == 0:
        return x
    elif i == 1:
        if x == 0:
            ...
            return 3*n - als_first_row.index(x)
    else:
        if x == 0:
            return n
        else:
            return n + als_first_row[n - x]

def bats_from_br(n: int):
    """Applies the Bose construction to given Bose resolution to obtain corresponding Bose-averaging triple system.
    Returns list of parallel classes of generated BATS, with points labelled according to Bose mapping/labelling."""
    br = get_bose_resolution(n)
    bats = [[] for _ in range(len(br) + 1)]
    # A list of triples in the BATS
    pc_current = [[] for _ in range(n)]
    als_first_row = [(x*(n+1)//2) % n for x in range(n)]
    for idx_1, lspc in enumerate(br):
        # each cell c in lspc gives rise to three triples in corresponding BATS
        for idx_2, c in enumerate(lspc):
            for i in range(3):
                pc_current[3*idx_2 + i] = [bose_mapping(n, c[0], i, als_first_row), bose_mapping(n, c[1], i, als_first_row), bose_mapping(n, c[2], (i + 1) % 3, als_first_row)]
        bats[idx_1] = pc_current.copy()

    bats[len(br)] = [[bose_mapping(n, x, i, als_first_row) for i in range(3)] for x in range(n)]
    return bats
